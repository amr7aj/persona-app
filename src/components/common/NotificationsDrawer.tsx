...
import { X, CheckCheck, Sparkles, Award, Target, Bell, ArrowRight } from 'lucide-react';
...
export const NotificationsDrawer: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-sm h-full bg-[#121217] border-l border-[#22222E] flex flex-col shadow-2xl">
        {/* Top bar */}
            ...
            notifications.map((n) => (
              <div
                ...
                className={`p-3 rounded-xl border transition-all cursor-pointer ${
                  n.read
                    ? 'bg-[#16161D] border-[#22222E] text-zinc-400'
                    : 'bg-[#1A1A24] border-purple-800/40 text-zinc-200 shadow-sm'
                }`}
              >
                <div className="flex items-start gap-2.5">
                  <div className="p-1.5 rounded-lg bg-[#22222E] mt-0.5">{getIcon(n.type)}</div>
